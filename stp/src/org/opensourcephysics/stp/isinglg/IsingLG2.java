  ...
  public void doOneMCStep() {
    for(int k = 0;k<np;++k) {
      ...
      if(inn > 0 && inn < (L+1) && lattice.getValue(inn,jnn)==0) { 
    	  ...
    	  int dE = 4-(lattice.getValue((inn+1),jnn)+lattice.getValue(inn-1,jnn)+lattice.getValue(inn,(jnn+1)%L)+lattice.getValue(inn,(jnn-1+L)%L));
    	  dE +=(lattice.getValue((x[n]+1),y[n])+lattice.getValue(x[n]-1,y[n])+lattice.getValue(x[n],(y[n]+1)%L)+lattice.getValue(x[n],(y[n]-1+L)%L));
        //restore
