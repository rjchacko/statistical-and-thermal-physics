...
public class HD implements Drawable
{
	...
	public double getHeatCapacity()
	{
		...
		double denom = sigma2
				/ (N * meanTemperature * meanTemperature) - 1.0;
		return N / denom;
	}
	...
	public void step()
	{
	    ...
	    for(int j = 0; j<10; j++) { // do 10 collisions
		  ...
		  totalKineticEnergySquaredAccumulator += getInstaneousKineticEnergy()
				* getInstaneousKineticEnergy();
	...
	public String toString()
	{
		...
		for (int i = 0; i < N; i++)
		{
			buffer.append(i + " x " + positionX[i] + " y " + positionY[i]
					+ " vx " + velocityX[i] + " vy " + velocityY[i] + "\n");
		}
	...
	public double getInstaneousKineticEnergy()
	{
		...
		for (int i = 0; i < N; i++)
		{
			kineticEnergy += velocityX[i] * velocityX[i] + velocityY[i]
					* velocityY[i];
		}
